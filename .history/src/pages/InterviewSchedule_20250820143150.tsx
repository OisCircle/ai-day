...
const InterviewSchedule: React.FC = () => {
  ...
  const generateAIResumeAnalysis = (candidate: Candidate) => {
    ...
    const overallScore = Math.round((experienceScore + skillsScore + projectScore) / 3 * 10) / 10;
    ...
    const matchRate = Math.round((matchedSkills.length / jobRequiredSkills.length) * 100);
    ...
    return {
      ...
      skillsScore,
      projectScore,
      recommendation: overallScore >= 7.5 ? "强烈推荐" : overallScore >= 6 ? "推荐" : "需要评估"
    };
  };
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-blue-600 uppercase tracking-wide">今日面试</p>
                </div>
                <p className="text-3xl font-bold text-gray-900 mb-2">{todayStats.total}</p>
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-green-600 uppercase tracking-wide">AI 评估完成</p>
                </div>
                <p className="text-3xl font-bold text-gray-900 mb-2">{todayStats.completed}</p>
                <div className="flex items-center text-sm text-gray-500">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-orange-600 uppercase tracking-wide">实时进行</p>
                </div>
                <p className="text-3xl font-bold text-gray-900 mb-2">{todayStats.inProgress}</p>
                <div className="flex items-center text-sm text-gray-500">
            ...
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center space-x-2 mb-3">
                  ...
                  <p className="text-sm font-semibold text-yellow-600 uppercase tracking-wide">AI 智能排期</p>
                </div>
                <p className="text-3xl font-bold text-gray-900 mb-2">{todayStats.scheduled}</p>
                <div className="flex items-center text-sm text-gray-500">
      ...
      <div className="card overflow-hidden">
        ...
        {filteredInterviews.length === 0 ? (
          <div className="px-6 py-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">该日期暂无面试安排</h3>
          ...
          <div className="space-y-4 p-6">
            {filteredInterviews.map((interview) => {
              ...
              const aiAnalysis = candidate ? generateAIResumeAnalysis(candidate) : null;
              
              return (
                <div key={interview.id} className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-all duration-300 hover:border-blue-300">
                        ...
                        <div className="relative">
                          <div className="h-12 w-12 rounded-full bg-gradient-to-r from-blue-400 to-purple-500 flex items-center justify-center">
                            <span className="text-white font-bold text-lg">
                              {getCandidateName(interview.candidateId).charAt(0)}
                            </span>
                          </div>
                          {aiAnalysis && (
                            <div className="absolute -bottom-1 -right-1 bg-white rounded-full p-1 shadow-lg">
                              <div className={`w-4 h-4 rounded-full ${
                                ...
                                aiAnalysis.overallScore >= 6 ? 'bg-yellow-500' : 'bg-red-500'
                              }`}></div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            <span className={`badge ${
                              interview.type === 'hr' ? 'badge-primary' :
                              interview.type === 'tech_1' ? 'badge-success' :
                              interview.type === 'tech_2' ? 'badge-warning' : 'badge-danger'
                            }`}>
                            ...
                            <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 mb-3">
                              <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className="text-sm font-semibold text-blue-800">AI 智能分析</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                  <span className={`text-sm font-bold ${
                                    aiAnalysis.overallScore >= 8 ? 'text-green-600' : 
                                    aiAnalysis.overallScore >= 6 ? 'text-yellow-600' : 'text-red-600'
                                  }`}>
                                  ...
                                  <div className="flex items-center">
                                    {[...Array(5)].map((_, i) => (
                                      <Star key={i} className={`h-3 w-3 ${
                                        i < Math.floor(aiAnalysis.overallScore/2) ? 'text-yellow-400 fill-current' : 'text-gray-300'
                                      }`} />
                                    ))}
                                  </div>
                                </div>
                              ...
                              <div className="grid grid-cols-3 gap-4 mb-3">
                                <div className="text-center">
                                  <div className="text-lg font-bold text-blue-600">{aiAnalysis.matchRate}%</div>
                                  <div className="text-xs text-gray-600">岗位匹配度</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-lg font-bold text-green-600">{aiAnalysis.experienceScore.toFixed(1)}</div>
                                  <div className="text-xs text-gray-600">经验评分</div>
                                </div>
                                <div className="text-center">
                                  <div className="text-lg font-bold text-purple-600">{aiAnalysis.skillsScore.toFixed(1)}</div>
                                  <div className="text-xs text-gray-600">技能评分</div>
                                </div>
                              </div>
                              ...
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-2">
                                  {aiAnalysis.insights.length > 0 && (
                                    <div className="flex space-x-1">
                                      {aiAnalysis.insights.slice(0, 2).map((insight, idx) => (
                                ...
                                <div className={`text-xs font-semibold px-3 py-1 rounded-full ${
                                  aiAnalysis.recommendation === '强烈推荐' ? 'bg-green-100 text-green-800' :
                                  aiAnalysis.recommendation === '推荐' ? 'bg-blue-100 text-blue-800' :
        ...
        <div className="card p-6 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">AI 智能操作</h3>
            </div>
            <div className="space-y-3">
              <button className="w-full btn btn-secondary justify-start group relative overflow-hidden">
                ...
                <div className="ml-auto bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full relative z-10">NEW</div>
              </button>
        ...
        <div className="card p-6 relative overflow-hidden">
          ...
          <div className="relative">
            <div className="flex items-center space-x-2 mb-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900">实时AI洞察</h3>
            </div>
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-green-800">候选人质量上升</p>
                    <p className="text-xs text-green-600">本周平均AI评分提升 +15%</p>
                  </div>
                </div>
              </div>
              <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-blue-800">面试效率优化</p>
                    <p className="text-xs text-blue-600">AI建议优化面试时长分配</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="relative">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-2">
                ...
                <h3 className="text-lg font-semibold text-gray-900">智能提醒</h3>
              </div>
            ...
            <div className="space-y-4">
              <div className="bg-gradient-to-r from-orange-50 to-yellow-50 rounded-lg p-4 border-l-4 border-orange-400">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-orange-800">30分钟后开始</p>
                    <p className="text-xs text-orange-600">陈小明 - 技术一面 (AI匹配度: 92%)</p>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 border-l-4 border-blue-400">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-blue-800">明日待安排</p>
                    <p className="text-xs text-blue-600">3场面试等待AI智能排期</p>
