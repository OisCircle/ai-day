...
const InterviewReports = () => {
    ...
    if (round.type === "hr") {
      ...
      dialogues.push(
        ...
        },
        {
          ...
          sentiment: "positive",
          keywords: ["自我介绍", "工作经验", "技术技能", "项目经验", "团队领导"],
        },
        {
          id: "d3",
        ...
        },
        {
          ...
          speaker: "candidate",
          content: "主要是希望能在一个更大的平台上发挥自己的能力，贵公司的技术实力和企业文化都很吸引我，我相信能在这里获得更好的职业发展。现在的公司规模相对较小，技术栈比较单一，我希望能接触更多前沿技术，也希望能在更大的业务体量下锻炼自己的架构设计能力。",
          sentiment: "positive",
        ...
        },
        {
          ...
          speaker: "candidate",
          content: "我了解贵公司是行业内的技术领先企业，在AI、大数据等方面有很强的技术积累。这个职位主要负责前端架构设计和团队管理，我希望能够运用我的技术经验，带领团队构建高质量的用户界面，同时也希望能在技术决策和团队建设方面发挥作用。我特别关注用户体验和性能优化，希望能在这方面有所贡献。",
          sentiment: "positive",
        ...
        },
        {
          ...
          speaker: "candidate",
          content: "薪资方面我希望能在25-30K之间，具体可以根据面试结果和公司标准来协商。关于工作时间，我理解互联网行业的特点，适度的加班是可以接受的，但我更倾向于通过提高工作效率和合理规划来平衡工作与生活。我相信高效的工作方式比简单的时间投入更重要。",
          sentiment: "positive",
      ...
      dialogues.push(
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 1).toISOString(),
          speaker: "candidate",
          content: "React的生命周期分为三个阶段：挂载、更新和卸载。挂载阶段包括constructor、componentDidMount等，更新阶段有componentDidUpdate、getSnapshotBeforeUpdate等，卸载阶段是componentWillUnmount。Hooks是React 16.8引入的新特性，它让我们可以在函数组件中使用状态和其他React特性。useState用于状态管理，useEffect用于副作用处理，相当于componentDidMount、componentDidUpdate和componentWillUnmount的组合。useEffect的依赖数组可以控制何时执行副作用。",
          sentiment: "positive",
          keywords: ["React生命周期", "useState", "useEffect", "函数组件", "副作用"],
        },
        {
          id: "d3",
          timestamp: addMinutes(timestamp, 3).toISOString(),
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 4).toISOString(),
          speaker: "candidate",
          content: "React性能优化有很多方法：1. 使用React.memo避免不必要的重渲染；2. 使用useMemo和useCallback缓存计算结果和函数；3. 代码分割和懒加载，使用React.lazy和Suspense；4. 虚拟列表处理大数据，如react-window；5. 使用生产环境构建；6. 避免在render中创建新对象和函数；7. 使用shouldComponentUpdate或PureComponent；8. 合理使用key prop；9. 图片懒加载和预加载。",
          sentiment: "positive",
          keywords: ["React.memo", "useMemo", "useCallback", "代码分割", "虚拟列表", "懒加载"],
        },
        {
          id: "d5",
          timestamp: addMinutes(timestamp, 6).toISOString(),
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 7).toISOString(),
          speaker: "candidate",
          content: "闭包是指函数能够访问其外部作用域的变量，即使外部函数已经返回。这创造了数据私有化的可能。原型链是JavaScript实现继承的机制，每个对象都有一个__proto__属性指向其原型。ES6引入了很多新特性：let/const块级作用域、箭头函数、模板字符串、解构赋值、Promise、async/await、class语法糖、模块化import/export、Set/Map数据结构等。这些特性大大提高了代码的可读性和开发效率。",
          sentiment: "positive",
          keywords: ["闭包", "原型链", "作用域", "箭头函数", "Promise", "async/await"],
        },
        {
          id: "d7",
          timestamp: addMinutes(timestamp, 9).toISOString(),
          speaker: "interviewer",
          content: "您在项目中使用过哪些状态管理方案？Redux和Context API有什么区别？",
          sentiment: "neutral",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 10).toISOString(),
          speaker: "candidate",
          content: "我使用过Redux、Context API、Zustand和Valtio等状态管理方案。Redux适合大型应用，提供了可预测的状态管理，但模板代码较多。Context API是React内置的，适合中小型应用或者跨组件传递数据，但性能上可能不如Redux。Redux有中间件机制，支持异步处理，而Context在频繁更新时可能导致不必要的重渲染。在最近的项目中，我倾向于使用Zustand，它更轻量且API更简洁。",
          sentiment: "positive",
        ...
        },
        {
          ...
          speaker: "candidate",
          content: "防抖是延迟执行，在事件停止触发一段时间后才执行；节流是限制执行频率，在一定时间内最多执行一次。防抖适用于搜索框输入、按钮点击；节流适用于滚动事件、鼠标移动。防抖函数实现：function debounce(func, delay) { let timer; return function() { clearTimeout(timer); timer = setTimeout(() => func.apply(this, arguments), delay); }; }",
          sentiment: "positive",
      ...
      dialogues.push(
        {
          ...
          timestamp: timestamp.toISOString(),
          speaker: "interviewer",
          content: "欢迎来到技术二面，今天我们主要讨论系统设计和架构相关的问题。请设计一个高并发的电商系统架构。",
          sentiment: "neutral",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 1).toISOString(),
          speaker: "candidate",
          content: "电商系统需要考虑高并发、高可用和数据一致性。我会采用微服务架构，将系统拆分为用户服务、商品服务、订单服务、支付服务等。前端使用CDN加速，后端使用负载均衡器分发请求。数据库采用读写分离，热点数据使用Redis缓存。对于秒杀等高并发场景，可以使用消息队列异步处理，限流熔断保护系统稳定性。",
          sentiment: "positive",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 4).toISOString(),
          speaker: "candidate",
          content: "CAP理论指出分布式系统无法同时保证一致性(Consistency)、可用性(Availability)和分区容错性(Partition tolerance)。实际应用中通常选择CP或AP。数据一致性可以通过分布式事务、消息队列、事件溯源等方式保证。我倾向于使用最终一致性，通过补偿机制确保数据最终达到一致状态，这样既保证了性能又保证了数据正确性。",
          sentiment: "positive",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 7).toISOString(),
          speaker: "candidate",
          content: "前端性能优化包括：1.资源优化：压缩、合并、CDN；2.渲染优化：减少DOM操作、虚拟滚动；3.网络优化：HTTP/2、预加载、缓存；4.代码优化：Tree shaking、懒加载。监控方面使用Performance API、LightHouse、Web Vitals等工具。排查时关注首屏时间、加载时间、运行时性能，通过Chrome DevTools分析瓶颈。",
          sentiment: "positive",
      ...
      dialogues.push(
        {
          ...
          timestamp: timestamp.toISOString(),
          speaker: "interviewer",
          content: "欢迎参加最终面试，我主要想了解您的技术规划和团队管理理念。您如何看待技术团队的发展方向？",
          sentiment: "neutral",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 1).toISOString(),
          speaker: "candidate",
          content: "我认为技术团队的发展应该紧密结合业务目标。首先要建立技术标准和最佳实践，确保代码质量；其次要关注团队成员的技术成长，通过技术分享、code review等方式提升整体水平；最后要保持对新技术的敏感度，但不盲目追求新技术，要结合业务场景选择合适的技术栈。我倾向于建设学习型团队，鼓励创新和试错。",
          sentiment: "positive",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 4).toISOString(),
          speaker: "candidate",
          content: "我会从技术能力、学习能力、沟通协作和价值观四个维度评估。技术能力看基础知识和实践经验；学习能力看对新技术的接受度和自驱力；沟通协作看表达能力和团队融入度；价值观看是否认同公司文化。我倾向于招聘有潜力的人才，通过mentorship帮助他们快速成长，而不是只看当前的技术水平。",
          sentiment: "positive",
          keywords: ["技术能力", "学习能力", "沟通协作", "价值观", "mentorship"],
        },
        {
          id: "d5",
          timestamp: addMinutes(timestamp, 6).toISOString(),
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 7).toISOString(),
          speaker: "candidate",
          content: "从了解到的情况来看，贵公司技术栈比较现代化，但我觉得可以在以下方面优化：1.建设更完善的监控体系，及时发现和解决问题；2.加强自动化测试，提高发布质量；3.推进DevOps实践，提升交付效率；4.建设技术中台，避免重复开发。这些都需要逐步推进，我愿意在这方面贡献力量。",
          sentiment: "positive",
  ...
  const generateAIAnalysisForDialogue = (
    ...
    return dialogues.map((dialogue, index) => {
      const analysisType = index % 4 === 0 ? "sentiment" : index % 4 === 1 ? "keyword" : index % 4 === 2 ? "competency" : "concern";
      let content = "";

      if (dialogue.speaker === "candidate") {
        switch (analysisType) {
          case "sentiment":
            const sentimentTemplates = sentimentAnalysisTemplates[dialogue.sentiment || "neutral"];
            content = sentimentTemplates[Math.floor(Math.random() * sentimentTemplates.length)];
            break;
          case "keyword":
            content = keywordAnalysisTemplates[Math.floor(Math.random() * keywordAnalysisTemplates.length)];
            break;
          case "competency":
            content = competencyAnalysisTemplates[Math.floor(Math.random() * competencyAnalysisTemplates.length)];
            break;
          case "concern":
            content = concernAnalysisTemplates[Math.floor(Math.random() * concernAnalysisTemplates.length)];
            break;
          default:
            content = candidateAnalysisTemplates[Math.floor(Math.random() * candidateAnalysisTemplates.length)];
        }
      } else {
        content = interviewerAnalysisTemplates[Math.floor(Math.random() * interviewerAnalysisTemplates.length)];
      }
  ...
  const handleViewMeetingRecord = (
    ...
    console.log('查找会议记录:', { candidateId, interviewType });
    console.log('当前会议记录数量:', meetingRecords.size);
    console.log('会议记录键值:', Array.from(meetingRecords.keys()));
    
    // 查找对应的会议记录
    const recordKey = Array.from(meetingRecords.keys()).find((key) => {
      const record = meetingRecords.get(key);
      console.log('检查记录:', { key, candidateId: record?.candidateId, interviewType: record?.interviewType });
      return (
    ...
    if (recordKey) {
      ...
      // 如果找不到会议记录，生成一个临时记录
      console.log('未找到会议记录，生成临时记录');
      const candidate = candidates.find(c => c.id === candidateId);
      if (candidate) {
        const tempDialogues = generateDialogueForRound(
          { type: interviewType, candidateId, actualStartTime: new Date().toISOString() },
          candidate
        );
