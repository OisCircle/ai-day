...
const InterviewReports = () => {
  ...
  const [selectedMeetingRecord, setSelectedMeetingRecord] = useState<InterviewMeetingRecord | null>(null);
  const [aiInsights, setAiInsights] = useState<Map<string, AIInsight>>(
    new Map()
  );
  const [meetingRecords, setMeetingRecords] = useState<Map<string, InterviewMeetingRecord>>(
    new Map()
  );
  ...
  const generateMeetingRecords = () => {
    ...
    interviewRoundsData.forEach((round) => {
      const candidate = candidatesData.find(c => c.id === round.candidateId);
      if (!candidate) return;

      // 生成模拟对话记录
      const dialogues: InterviewDialogue[] = generateDialogueForRound(round, candidate);
      
      // 生成AI分析
      const aiAnalysis: AIDialogueAnalysis[] = generateAIAnalysisForDialogue(dialogues);
      ...
      const record: InterviewMeetingRecord = {
        ...
        duration: calculateDuration(round.actualStartTime || round.scheduledTime, round.actualEndTime || round.scheduledTime),
        dialogues,
        aiAnalysis,
        summary: {
          keyTopics: generateKeyTopics(round.type),
          candidateStrengths: generateStrengthsFromDialogue(dialogues, round.type),
          concerns: generateConcernsFromDialogue(dialogues, round.type),
          aiInsights: generateAIInsightsFromDialogue(dialogues, round.type),
  ...
  // 辅助函数：生成对话内容
  const generateDialogueForRound = (round: InterviewRound, candidate: any): InterviewDialogue[] => {
    ...
    if (round.type === "hr") {
      ...
      dialogues.push(
        ...
        },
        {
          ...
          speaker: "candidate",
          content: `我叫${candidate.name}，有${candidate.resume.experience.length}年的工作经验，主要专注于${candidate.resume.skills.technical.slice(0, 3).join("、")}等技术领域。我在${candidate.resume.experience[0]?.company}工作期间，负责了多个重要项目的开发...`,
          sentiment: "positive",
        ...
        },
        {
          ...
          speaker: "interviewer", 
          content: "很好，您的经验很丰富。为什么想要离开目前的公司，选择加入我们呢？",
          sentiment: "neutral",
          keywords: ["离职原因", "选择公司"]
        },
        {
          ...
          speaker: "candidate",
          content: "主要是希望能在一个更大的平台上发挥自己的能力，贵公司的技术实力和企业文化都很吸引我，我相信能在这里获得更好的职业发展。",
          sentiment: "positive", 
      ...
      dialogues.push(
        {
          ...
          timestamp: timestamp.toISOString(),
          speaker: "interviewer",
          content: "我们开始技术面试，首先请您介绍一下React的生命周期，以及Hooks的工作原理。",
          sentiment: "neutral",
        ...
        },
        {
          ...
          timestamp: addMinutes(timestamp, 1).toISOString(),
          speaker: "candidate",
          content: "React的生命周期分为三个阶段：挂载、更新和卸载。Hooks是React 16.8引入的新特性，它让我们可以在函数组件中使用状态和其他React特性。useState用于状态管理，useEffect用于副作用处理...",
          sentiment: "positive",
        ...
        },
        {
          ...
          speaker: "candidate", 
          content: "React性能优化有很多方法：1. 使用React.memo避免不必要的重渲染；2. 使用useMemo和useCallback缓存计算结果和函数；3. 代码分割和懒加载；4. 虚拟列表处理大数据；5. 使用生产环境构建...",
          sentiment: "positive",
          keywords: ["React.memo", "useMemo", "useCallback", "代码分割", "虚拟列表"]
        }
      );
    }
  ...
  // 生成AI对话分析
  const generateAIAnalysisForDialogue = (dialogues: InterviewDialogue[]): AIDialogueAnalysis[] => {
    return dialogues.map((dialogue, index) => ({
      id: `analysis_${dialogue.id}`,
      timestamp: dialogue.timestamp,
      analysisType: index % 4 === 0 ? "sentiment" : index % 4 === 1 ? "keyword" : index % 4 === 2 ? "competency" : "concern",
      content: dialogue.speaker === "candidate" 
  ...
  const generateStrengthsFromDialogue = (dialogues: InterviewDialogue[], type: string) => {
    return [
      "表达逻辑清晰",
      "专业知识扎实", 
      "学习能力强",
  ...
  const generateConcernsFromDialogue = (dialogues: InterviewDialogue[], type: string) => {
    return [
      "某些技术细节需要深入",
      "项目管理经验可以加强"
    ];
  ...
  const generateAIInsightsFromDialogue = (dialogues: InterviewDialogue[], type: string) => {
    return [
      "候选人技术基础扎实，逻辑思维清晰",
      "沟通表达能力强，适合团队协作",
      "学习意愿强烈，有良好的成长潜力"
  ...
  const generateNextSteps = (type: string, recommendation: string) => {
  ...
  const handleViewMeetingRecord = (candidateId: string, interviewType: string) => {
    // 查找对应的会议记录
    const recordKey = Array.from(meetingRecords.keys()).find(key => {
      const record = meetingRecords.get(key);
      return record?.candidateId === candidateId && record?.interviewType === interviewType;
    });
  ...
  const getMeetingRecordByCandidate = (candidateId: string) => {
    return Array.from(meetingRecords.values()).filter(record => record.candidateId === candidateId);
  };
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            {/* 会议记录按钮 */}
                            <button
                              onClick={() => handleViewMeetingRecord(selectedCandidate.id, report.interviewType)}
                        ...
                        <div className="mt-3 flex items-center justify-between">
                          ...
                          <button
                            onClick={() => handleViewMeetingRecord(candidate.id, report.interviewType)}
          ...
          <div className="relative top-10 mx-auto p-5 border w-11/12 max-w-7xl shadow-lg rounded-md bg-white">
              ...
              <div className="flex items-center">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-gray-900">
                    面试会议记录 - {selectedMeetingRecord.interviewType === "hr" ? "HR面试" : 
                    selectedMeetingRecord.interviewType === "tech_1" ? "技术一面" :
                    selectedMeetingRecord.interviewType === "tech_2" ? "技术二面" : "VP面试"}
                  </h3>
                  <p className="text-sm text-gray-600 mt-1 flex items-center">
                    <Clock className="h-4 w-4 mr-1" />
                    {new Date(selectedMeetingRecord.startTime).toLocaleString("zh-CN")} • 
                    时长 {selectedMeetingRecord.duration} 分钟 • 面试官: {selectedMeetingRecord.interviewerName}
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
              <div className="bg-gradient-to-r from-green-50 to-green-100 p-4 rounded-lg border border-green-200">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <div className="text-sm font-medium text-green-800">转录质量</div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-blue-100 p-4 rounded-lg border border-blue-200">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <div className="text-sm font-medium text-blue-800">AI置信度</div>
                    <div className="text-lg font-bold text-blue-600">
                      {Math.round(selectedMeetingRecord.transcription.confidence)}%
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-purple-50 to-purple-100 p-4 rounded-lg border border-purple-200">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <div className="text-sm font-medium text-purple-800">对话轮次</div>
              ...
              <div className="bg-gradient-to-r from-orange-50 to-orange-100 p-4 rounded-lg border border-orange-200">
                <div className="flex items-center">
                  ...
                  <div className="ml-3">
                    <div className="text-sm font-medium text-orange-800">AI分析</div>
                    <div className="text-lg font-bold text-orange-600">
              ...
              <div className="lg:col-span-2">
                <div className="card p-6">
                  ...
                  <div className="space-y-4 max-h-[600px] overflow-y-auto">
                    {selectedMeetingRecord.dialogues.map((dialogue, index) => (
                      <div key={dialogue.id} className="flex items-start space-x-4">
                        {/* 说话者头像 */}
                        ...
                        <div className="flex-1">
                          <div className="flex items-center space-x-2 mb-2">
                            <span className="text-sm font-medium text-gray-900">
                              {dialogue.speaker === "interviewer" ? "面试官" : "候选人"}
                            </span>
                            ...
                            {dialogue.sentiment && (
                              <span className={`text-xs px-2 py-1 rounded-full ${
                                dialogue.sentiment === "positive" ? "bg-green-100 text-green-700" :
                                dialogue.sentiment === "negative" ? "bg-red-100 text-red-700" :
                                "bg-gray-100 text-gray-700"
                              }`}>
                                {dialogue.sentiment === "positive" ? "积极" : 
                                 dialogue.sentiment === "negative" ? "消极" : "中性"}
                              </span>
                            )}
                          </div>
                          ...
                          <div className={`p-4 rounded-lg ${
                            ...
                            <p className="text-gray-700 leading-relaxed">{dialogue.content}</p>
                            
                            {dialogue.keywords && dialogue.keywords.length > 0 && (
                              <div className="flex flex-wrap gap-2 mt-3">
                                {dialogue.keywords.map((keyword, idx) => (
                                  <span key={idx} className="text-xs bg-white bg-opacity-60 text-gray-600 px-2 py-1 rounded-full border">
                                    {keyword}
                                  </span>
                                ))}
                              </div>
                          ...
                          {selectedMeetingRecord.aiAnalysis
                            .filter(analysis => analysis.relatedDialogue === dialogue.id)
                            .map(analysis => (
                              <div key={analysis.id} className="mt-3 bg-gradient-to-r from-purple-50 to-indigo-50 p-3 rounded-lg border border-purple-200">
                                <div className="flex items-start">
                                  <Bot className="h-4 w-4 text-purple-600 mr-2 mt-0.5 flex-shrink-0" />
                                  <div className="flex-1">
                                    <div className="flex items-center space-x-2 mb-1">
                                      <span className="text-xs font-medium text-purple-800">
                                        AI {analysis.analysisType === "sentiment" ? "情感分析" :
                                             analysis.analysisType === "keyword" ? "关键词提取" :
                                             analysis.analysisType === "competency" ? "能力评估" : "风险识别"}
                                      </span>
                                      <span className="text-xs bg-purple-100 text-purple-600 px-2 py-1 rounded-full">
                                        置信度 {Math.round(analysis.confidence)}%
                                      </span>
                                    </div>
                                    <p className="text-xs text-purple-700">{analysis.content}</p>
                                  </div>
                                </div>
                              </div>
                            ))}
                ...
                <div className="space-y-6">
                  ...
                  <div className="card p-5">
                    ...
                    <div className="space-y-2">
                      {selectedMeetingRecord.summary.keyTopics.map((topic, index) => (
                  ...
                  <div className="card p-5">
                    ...
                    <div className="space-y-2">
                      {selectedMeetingRecord.summary.candidateStrengths.map((strength, index) => (
                  ...
                  {selectedMeetingRecord.summary.concerns.length > 0 && (
                    <div className="card p-5">
                      ...
                      <div className="space-y-2">
                        {selectedMeetingRecord.summary.concerns.map((concern, index) => (
                          <div key={index} className="flex items-center text-sm bg-orange-50 p-2 rounded">
                  ...
                  <div className="card p-5">
                    ...
                    <div className="space-y-3">
                      {selectedMeetingRecord.summary.aiInsights.map((insight, index) => (
                        <div key={index} className="bg-gradient-to-r from-purple-50 to-indigo-50 p-3 rounded-lg border border-purple-200">
                          <div className="flex items-start">
                  ...
                  <div className="card p-5">
                    ...
                    <div className="space-y-2">
                      {selectedMeetingRecord.summary.nextSteps.map((step, index) => (
                        <div key={index} className="flex items-center text-sm bg-indigo-50 p-2 rounded">
            ...
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
              <div className="flex items-center text-sm text-gray-600">
                <Bot className="h-4 w-4 mr-2" />
                <span>AI分析于 {new Date().toLocaleDateString("zh-CN")} 生成</span>
              </div>
