...
import { InterviewRound, InterviewQuestion, Candidate, AIQuestionTemplate } from '../types';
...
const InterviewConducting: React.FC = () => {
  ...
  useEffect(() => {
    // 加载面试数据
    const interviewData = (interviewRoundsData as InterviewRound[]).find(i => i.id === id);
    if (interviewData) {
      setInterview(interviewData);
      const candidateData = (candidatesData as Candidate[]).find(c => c.id === interviewData.candidateId);
  ...
  const generateAIQuestions = async () => {
    ...
    const relevantTemplate = questionTemplates.find(t => 
      t.category === (interview.type === 'tech_1' ? 'technical' : 'behavioral')
    );

    if (relevantTemplate) {
      const generatedQuestions: InterviewQuestion[] = relevantTemplate.questions.map((q, index) => ({
        id: `ai_${index}`,
        question: q.question,
  ...
  const currentQuestions = aiQuestions.length > 0 ? aiQuestions : (interview?.questions || []);
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                {interview.type === 'tech_1' ? '技术一面' : interview.type === 'tech_2' ? '技术二面' : interview.type === 'hr' ? 'HR面试' : 'VP面试'}
              </h1>
          ...
          <div className="flex items-center space-x-3">
            {!interviewStarted ? (
              ...
              <>
                ...
                <button onClick={handleStopInterview} className="btn btn-danger">
          ...
          <div className="mt-4 p-4 bg-gray-50 rounded-lg">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center">
                  {isRecording ? (
                    <>
                      ...
                      <span className="text-sm font-medium text-red-700">录制中</span>
                    </>
                  ) : (
                    <>
                      <div className="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div>
                      <span className="text-sm font-medium text-yellow-700">已暂停</span>
                    </>
                  )}
                </div>
              ...
              <div className="flex items-center space-x-2">
                <button className={`btn btn-sm ${isRecording ? 'btn-danger' : 'btn-secondary'}`}>
          ...
          <div className="card p-4">
            ...
            {isGeneratingQuestions && (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500">AI正在根据候选人简历生成面试问题...</p>
              </div>
            )}
        ...
        <div className="lg:col-span-2 space-y-6">
          {currentQuestion && (
            <>
              ...
              <div className="card p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <span className={`badge ${
                      ...
                      currentQuestion.category === 'behavioral' ? 'badge-success' : 'badge-warning'
                    }`}>
                ...
                {currentQuestion.keyPoints && currentQuestion.keyPoints.length > 0 && (
                  <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                    <h4 className="text-sm font-medium text-blue-900 mb-2">评估关键点:</h4>
                ...
                {currentQuestion.followUpQuestions && currentQuestion.followUpQuestions.length > 0 && (
                  <div className="mt-4">
                    <h4 className="text-sm font-medium text-gray-900 mb-2">可能的追问:</h4>
                    <div className="space-y-2">
              ...
              {/* 候选人回答记录 */}
              <div className="card p-6">
                <h4 className="text-lg font-medium text-gray-900 mb-4">候选人回答</h4>
                <textarea
                ...
                <h4 className="text-lg font-medium text-gray-900 mb-4">面试官备注</h4>
                <textarea
              ...
              {interviewStarted && (
                <div className="card p-6">
                  <h4 className="text-lg font-medium text-gray-900 mb-4">实时评估</h4>
