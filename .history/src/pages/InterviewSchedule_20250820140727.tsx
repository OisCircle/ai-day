...
import { InterviewRound, Candidate, User as UserType } from '../types';
...
const InterviewSchedule: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    const statusMap = {
      ...
      in_progress: { text: '进行中', className: 'badge-primary', icon: AlertCircle },
      completed: { text: '已完成', className: 'badge-success', icon: CheckCircle },
      cancelled: { text: '已取消', className: 'badge-danger', icon: AlertCircle }
    };
    const statusInfo = statusMap[status as keyof typeof statusMap] || { text: status, className: 'badge-gray', icon: AlertCircle };
  ...
  const filteredInterviews = interviews.filter(interview => {
    ...
    const interviewDate = new Date(interview.scheduledTime).toISOString().split('T')[0];
  ...
  const todayStats = {
    ...
    inProgress: todayInterviews.filter(i => i.status === 'in_progress').length,
