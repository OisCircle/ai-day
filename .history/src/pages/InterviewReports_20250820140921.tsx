...
const InterviewReports: React.FC = () => {
  ...
  const generateMockReport = (candidate: Candidate) => {
    const completedInterviews = interviews.filter(i => 
      i.candidateId === candidate.id && i.status === 'completed'
    );
    ...
    const reports = completedInterviews.map(interview => ({
      ...
      strengths: interview.type === 'tech_1' 
        ? ['React技术栈掌握扎实', '系统设计思路清晰', '代码质量高']
        : ['沟通表达能力强', '学习能力突出', '团队协作意识好'],
      weaknesses: interview.type === 'tech_1'
        ? ['部分新技术了解不够深入', '性能优化经验需要加强']
        : ['项目管理经验相对不足'],
      detailedFeedback: interview.type === 'tech_1'
  ...
  const getRecommendationBadge = (recommendation: string) => {
    ...
    const info = map[recommendation as keyof typeof map] || { text: recommendation, class: 'badge-gray' };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredCandidates.map((candidate) => {
          ...
          const avgScore = mockReports.length > 0 
                      ...
                      <div>
                        <div className="text-sm font-medium text-gray-900">
                          {report.interviewType === 'hr' ? 'HR面试' : 
                           ...
                           report.interviewType === 'tech_2' ? '技术二面' : 'VP面试'}
                        </div>
              ...
              <div key={report.id} className="mb-8 card p-6">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-lg font-medium text-gray-900">
                    {report.interviewType === 'hr' ? 'HR面试报告' : 
                     report.interviewType === 'tech_1' ? '技术一面报告' : 
                     report.interviewType === 'tech_2' ? '技术二面报告' : 'VP面试报告'}
                  </h4>
                  <div className="flex items-center space-x-4">
                    <span className={`text-xl font-bold ${getScoreColor(report.overallScore)}`}>
                      {report.overallScore}/10
                    </span>
                    {getRecommendationBadge(report.recommendation)}
                  </div>
                ...
                <div className="mt-6 pt-4 border-t flex items-center justify-between text-sm text-gray-500">
                  ...
                  <div>面试时间: {new Date(report.date).toLocaleDateString('zh-CN')}</div>
                </div>
              </div>
            ))}
