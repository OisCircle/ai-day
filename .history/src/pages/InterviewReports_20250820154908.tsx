            ...
            {aiInsights.get(selectedCandidate.id) && (
              <div className="mb-8 relative overflow-hidden">
                ...
                <div className="card p-8 bg-gradient-to-r from-indigo-50 via-blue-50 to-purple-50 border-2 border-blue-300 relative z-10">
                  <div className="flex items-center justify-between mb-6">
                    <h4 className="text-2xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent flex items-center">
                      <Brain className="h-7 w-7 text-blue-500 mr-3" />
                      AI全维度智能分析报告
                    </h4>
                    <div className="flex items-center space-x-2">
                      <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                      <span className="text-sm text-green-600 font-medium">AI实时分析</span>
                    </div>
                  </div>

                  {/* AI核心指标展示 */}
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                    <div className="text-center p-4 bg-white bg-opacity-80 rounded-xl shadow-sm border border-blue-200">
                      ...
                      <div className="text-sm font-medium text-gray-700">AI综合评分</div>
                      <div className="text-xs text-blue-500 mt-1">满分10分</div>
                      {/* 进度环 */}
                      <div className="mt-3 relative w-12 h-12 mx-auto">
                        <div className="w-12 h-12 rounded-full border-4 border-blue-100"></div>
                        <div 
                          className="absolute top-0 left-0 w-12 h-12 rounded-full border-4 border-blue-500 transform -rotate-90 transition-all duration-1000"
                          style={{
                            clipPath: `polygon(50% 50%, 50% 0%, ${50 + 50 * Math.cos(2 * Math.PI * (aiInsights.get(selectedCandidate.id)!.aiScore / 10) - Math.PI / 2)}% ${50 + 50 * Math.sin(2 * Math.PI * (aiInsights.get(selectedCandidate.id)!.aiScore / 10) - Math.PI / 2)}%, 50% 50%)`
                          }}
                        ></div>
                      </div>
                    </div>

                    <div className="text-center p-4 bg-white bg-opacity-80 rounded-xl shadow-sm border border-green-200">
                      ...
                      <div className="text-sm font-medium text-gray-700">文化匹配度</div>
                      <div className="text-xs text-green-500 mt-1">
                        {aiInsights.get(selectedCandidate.id)!.culturalFit >= 90 ? '极佳匹配' : 
                         aiInsights.get(selectedCandidate.id)!.culturalFit >= 80 ? '高度匹配' : '良好匹配'}
                      </div>
                      <div className="mt-3 w-full bg-green-100 rounded-full h-2">
                        <div 
                          className="bg-gradient-to-r from-green-400 to-green-600 h-2 rounded-full transition-all duration-1000"
                          style={{width: `${aiInsights.get(selectedCandidate.id)!.culturalFit}%`}}
                        ></div>
                      </div>
                    </div>

                    <div className="text-center p-4 bg-white bg-opacity-80 rounded-xl shadow-sm border border-purple-200">
                      ...
                      <div className="text-sm font-medium text-gray-700">AI置信度</div>
                      <div className="text-xs text-purple-500 mt-1">
                        {aiInsights.get(selectedCandidate.id)!.confidence >= 95 ? '极高置信' : 
                         aiInsights.get(selectedCandidate.id)!.confidence >= 85 ? '高置信度' : '中等置信'}
                      </div>
                        ...
                        {[...Array(5)].map((_, i) => (
                          <Sparkles 
                            ...
                            className={`h-3 w-3 mx-0.5 ${
                              i < Math.floor(aiInsights.get(selectedCandidate.id)!.confidence / 20) 
                                ? 'text-purple-500' 
                                : 'text-gray-300'
                            }`} 
                          />
                    ...
                    <div className="text-center p-4 bg-white bg-opacity-80 rounded-xl shadow-sm border border-orange-200">
                      ...
                      <div className="text-sm font-medium text-gray-700">风险评估</div>
                      <div className="text-xs text-orange-500 mt-1">AI安全评级</div>
                      <div className="mt-3">
                        <div className={`w-8 h-8 mx-auto rounded-full flex items-center justify-center ${
                          aiInsights.get(selectedCandidate.id)!.riskLevel === "low" ? 'bg-green-500' :
                          aiInsights.get(selectedCandidate.id)!.riskLevel === "medium" ? 'bg-yellow-500' : 'bg-red-500'
                        }`}>
                  ...
                  <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-6 rounded-xl text-white mb-6">
                    <div className="flex items-start">
                      <div className="p-2 bg-white bg-opacity-20 rounded-lg mr-4 mt-1">
                      ...
                      <div className="flex-1">
                        <h5 className="text-lg font-bold mb-2 flex items-center">
                          ...
                          <span className="ml-2 text-xs bg-white bg-opacity-20 px-2 py-1 rounded-full">
                            置信度 {aiInsights.get(selectedCandidate.id)!.confidence}%
                          </span>
                        </h5>
                        ...
                        <div className="mt-4 flex items-center">
                          <span className="text-sm font-medium mr-3">AI推荐等级:</span>
