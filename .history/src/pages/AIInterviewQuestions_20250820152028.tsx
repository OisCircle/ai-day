...
const AIInterviewQuestions = () => {
  ...
  useEffect(() => {
    // 加载问题模板数据并扩展
    const processedQuestions: Question[] = (questionTemplatesData as any[]).map(
      (template, index) => ({
        id: `q-${index}`,
      ...
      const newQuestions: Question[] = [
        {
          ...
          difficulty: "medium",
          question:
            "请描述React中useState和useEffect的执行时机，以及它们之间的关系？",
            ...
            <div className="card p-4">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-gray-900">
                    {Math.round(
                      (questions.reduce((acc, q) => acc + q.averageScore, 0) /
                        questions.length) *
                        10
                    ) / 10}
                  </div>
            ...
            <div className="card p-4">
              <div className="flex items-center">
                ...
                <div>
                  <div className="text-2xl font-bold text-gray-900">
                    {Math.round(
                      questions.reduce((acc, q) => acc + q.effectiveness, 0) /
                        questions.length
                    )}
                    %
                  </div>
